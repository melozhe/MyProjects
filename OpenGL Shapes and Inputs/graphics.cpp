...
void object::draw(size_t num, bool is_selected) {
   pshape->draw (center, color, num);
   if (is_selected)
      pshape->draw_border(center, window::bordercolor, window::borderwidth);
}
...
void object::move (GLfloat delta_x, GLfloat delta_y) {
   ...
   else if (center.xpos <= 0) center.xpos = window::width;
   else if (center.ypos > window::height) center.ypos = 0;
   else if (center.ypos <= 0 )center.ypos = window::height;
}
...
void window::display() {
   ...
   for (size_t i=0; i < window::objects.size(); ++i) {
      if (i == window::selected_obj){
         window::objects.at(i).draw(i, true);         
      }else{
...
void window::move_selected_obj(int dx, int dy){
   window::objects.at(selected_obj).move(dx, dy);
}
...
void window::keyboard (GLubyte key, int x, int y) {
   ...
   switch (key) {
      ...
      case 'H': case 'h':
         ...
         move_selected_obj(-window::moveby,0);
         break;
      case 'J': case 'j':
         //move_selected_object (
         move_selected_obj(0,-window::moveby);
         break;
      case 'K': case 'k':
         //move_selected_object (
         move_selected_obj(0,window::moveby);
